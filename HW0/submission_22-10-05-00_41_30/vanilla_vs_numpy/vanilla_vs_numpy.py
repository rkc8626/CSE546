...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    '''
        - bad code 
    result = []  # Create list of d lists
    for n in range(0, len(A), 1):
        for i in range(0, n, 1):
            cursum = x[i]*A[n][i] + x[i]*A[i][n] + y[i]*B[i][n]
            result.append(cursum)
    return result
    ''' 
    M1 = vanilla_matmul(A,x)  
    M2 = vanilla_matmul(vanilla_transpose(A),x)  
    M3 = vanilla_matmul(vanilla_transpose(B),y)
...
def numpy_solution(
    ...
    """    
    return (A @ x + np.transpose(A) @ x + np.transpose(B) @ y)
